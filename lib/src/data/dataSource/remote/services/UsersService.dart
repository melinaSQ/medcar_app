...
class UsersService {
  ...
  //     int id, String notificationToken) async {
  //   try {
  //     Uri url =
  //         Uri.http(ApiConfig.API_PROJECT, '/users/notification_token/$id');
  //     Map<String, String> headers = {
  //       'Content-Type': 'application/json',
  //     };
  //     String body = json.encode({
  ...
  Future<Resource<User>> updateImage(int id, User user, File file) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        var errord = ErrorData(listToString(data['message']));
        print('Error: ${errord.message}');
        return ErrorData(listToString(data['message']));
      }
    } catch (e) {
      print('Error: $e');
